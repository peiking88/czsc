...
│   ├── signals.*         ← 222 个信号函数（Python 端 7 个子模块；详见 crates/czsc-signals/src/）
...
* 定义并实现了 222 个信号函数（Rust 实现），见 `czsc._native.signals`
...
> **Rust 构建环境约束**：底层依赖 `pyo3` 0.28 要求 Python ≥ 3.10（编译期校验）。
> 但 `pyproject.toml` 声明 `requires-python = ">=3.12"`，发布包仅支持 3.12+。
> 当系统默认 Python 低于 3.10 时，构建会提前 panic：
>
> ```bash
> export PYO3_PYTHON=$(which python3.12)   # 或任意 3.10+ 的解释器
> ```
>
> 用 `uv sync` 走 UV 流程时，UV 会自动选择项目声明的 Python，不需要额外设置。
...
| 5 | [信号函数模块深入介绍](https://s0cqcxuy3p.feishu.cn/wiki/WxU8wKTBViekKbkCyKzcwa7SnJd) | 系统讲解 czsc-signals 架构与 222 个信号函数的模块划分、参数模板、信号表达示例。 |
...
```bash
# 使用 UV 管理依赖（推荐）
# --group dev 走 [dependency-groups]（含 pytest/ruff/basedpyright 等）
uv sync --group dev
...
- 文档: 修正 README/CLAUDE 中 Python版本、pyo3版本、信号函数数量、依赖管理命令与实际一致

### 1.7.8 (2026-06-29)

- 重构: 预测与合并脚本改用 TDengine 直读，去除 tdxdata/mootdx/opentdx 依赖
- 新增: 合并脚本去除头条博主、新增广发郭磊分析师
- 修复: TDX 复权因子网络获取失败时用过期缓存兜底
- 新增: 盘面分析自动生成与报告导航优化
- 新增: Kronos+CZSC 合并报告增强与分型强度判定优化
- 优化: 强分型+买卖点重点标注与增强分型强度判定
- 修复: predict.py 分型判定错误与 pandas 3.0 兼容
- 新增: Kronos+CZSC 联合分析报告合并脚本
- 新增: 多周期强分型+买卖点扫描工具
- 新增: 三周期仓位提醒与报告优化
