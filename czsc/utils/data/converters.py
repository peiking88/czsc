...
def to_standard_kline_format(
    ...
    for old_col, new_col in column_mapping.items():
        if old_col in df.columns:
            ...
            warnings.warn(f"列 '{old_col}' 不存在于输入DataFrame中", stacklevel=2)
...
def pivot_weight_data(dfw: pd.DataFrame, fill_value: float = 0.0) -> pd.DataFrame:
    ...
    pivot_df = pd.pivot_table(df, index="dt", columns="symbol", values="weight", aggfunc="sum")
...
def resample_to_period(df: pd.DataFrame, period: str, agg_dict: dict[str, str] | None = None) -> pd.DataFrame:
    ...
    if agg_dict is None:
        # 默认聚合方式
        agg_dict = dict.fromkeys(df.columns, "sum")
