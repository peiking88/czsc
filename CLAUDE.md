...
# --extra dev 走 [project.optional-dependencies]；--group dev 走 [dependency-groups]
uv sync --group dev
...
1. **`czsc._native`** - PyO3 编译产生的 Rust 扩展模块（缠论核心）：
   ...
   - 暴露 222 个信号函数（Python 端 7 个子模块：bar/cvolp/cxt/obv/pressure/tas/vol；底层 `crates/czsc-signals/src/` 有 24 个 .rs 源文件，可用 `ls crates/czsc-signals/src/` 自查最新清单）
...
4. **`czsc/_native.signals`** - 信号函数（Rust 实现，通过 PyO3 暴露）：
   - Python 端暴露 7 个子模块（bar/cvolp/cxt/obv/pressure/tas/vol）；底层 `crates/czsc-signals/src/` 有 24 个 .rs 源文件，自查命令：`ls crates/czsc-signals/src/`
...
- **构建环境约束**：`pyo3-stub-gen` 与 `pyo3` 0.28 都要求 Python ≥ 3.10（编译期校验）；`pyproject.toml` 声明 `requires-python = ">=3.12"`，发布包仅支持 3.12+。通过 `crates/czsc-python/build.rs` 在编译期校验 `PYO3_PYTHON`，低于 3.10 时直接报错
