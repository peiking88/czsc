...
1. 运行 `UV_INDEX_URL=https://pypi.tuna.tsinghua.edu.cn/simple uv sync --extra dev`
...
| 依赖下载慢 | PyPI 默认源在国外 | 已在 `pyproject.toml` `[tool.uv]` 配置清华镜像；如仍慢可临时用 `UV_INDEX_URL=https://pypi.tuna.tsinghua.edu.cn/simple` |
